...
export function StatsSidebar() {
  ...
  const columnCounts = columns.map((col) => {
    ...
    const color = KNOWN_COLORS[col.id] ?? TAILWIND_TO_HEX[col.color];
  ...
  const chartData =
    total === 0
      ...
      : columnCounts.filter((c) => c.count > 0).map((c) => ({
          name: c.label,
          value: c.count,
          color: c.color,
        }));
  ...
  function formatDueLabel(dueDateStr: string): {
    ...
    if (diffDays < 0) return { label: `${Math.abs(diffDays)}d overdue`, isOverdue: true, isDueToday: false };
    if (diffDays === 0) return { label: "Due today", isOverdue: false, isDueToday: true };
    if (diffDays === 1) return { label: "Due tomorrow", isOverdue: false, isDueToday: false };
    if (diffDays <= 7) return { label: `In ${diffDays} days`, isOverdue: false, isDueToday: false };
    return {
      ...
      <div className="flex flex-col gap-3 p-3">
        <div className="rounded-xl border bg-background shadow-sm overflow-hidden">
          <div className="px-3 py-2.5 border-b border-dashed">
            <p className="text-xs font-semibold text-foreground leading-none">Completion</p>
          ...
          <div className="px-3 py-3 flex flex-col items-center gap-3">
                  ...
                  <PieChart>
                    ...
                    >
                      {chartData.map((entry, index) => (
                        <Cell key={`cell-${index}`} fill={total === 0 ? "#e2e8f0" : entry.color} />
                      ))}
                    </Pie>
                    <Tooltip
            ...
            <div className="w-full flex flex-col gap-2">
              {columnCounts.map((col) => {
                const pct = total === 0 ? 0 : Math.round((col.count / total) * 100);
                return (
                  <div key={col.id} className="flex flex-col gap-0.5">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[10px] text-muted-foreground truncate max-w-20">{col.label}</span>
                      </div>
                      <span className="text-[10px] font-bold tabular-nums">{mounted ? col.count : "–"}</span>
                    </div>
                    <div className="h-0.5 w-full rounded-full bg-muted overflow-hidden">
                      <div
                        className="h-full rounded-full transition-all duration-500"
                        style={{ width: mounted ? `${pct}%` : "0%", background: col.color }}
                      />
                    </div>
                  </div>
                );
        ...
        <div className="rounded-xl border bg-background shadow-sm overflow-hidden">
          <div className="px-3 py-2.5 border-b border-dashed flex items-center gap-1.5">
            ...
            <p className="text-xs font-semibold text-foreground leading-none">Due Soon</p>
          </div>
          ...
          {!mounted ? (
            ...
            <div className="flex flex-col items-center justify-center py-6 px-3 gap-1.5">
              ...
              <p className="text-[10px] text-muted-foreground text-center leading-relaxed">
                All caught up!<br />No pending tasks.
              </p>
            </div>
          ) : (
            <div className="flex flex-col">
              {upcomingTasks.map((task, index) => {
                const { label, isOverdue, isDueToday } = formatDueLabel(task.dueDate);
                return (
                  <div
                    key={task.id}
                    className={`px-3 py-2.5 flex flex-col gap-0.5 ${index !== upcomingTasks.length - 1 ? "border-b" : ""}`}
                  >
                    <div className="flex items-center gap-1.5">
                      {isOverdue ? (
                    ...
                    <div className="flex items-center justify-between pl-3">
                      <span className={`text-[10px] font-medium ${isOverdue ? "text-red-500" : isDueToday ? "text-amber-500" : "text-muted-foreground"}`}>
                        {label}
                      </span>
                      <span
